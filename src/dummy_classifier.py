...
def main():
    ...
    trained_musical_classifier = DummyClassifier().fit(df.values, labels.values)
    ...
    print(f'Accuracy count for Dummy Classifier is: {(correct_count / len(labels)) * 100}%')

    with open('output_dummy_dev.csv', 'w') as file:
    ...
    print(f'Accuracy count for Dummy {iterations} iterations on TEST set is: {(correct_count / len(labels)) * 100}%')

    with open('output_dummy_test.csv', 'w') as file:
        csv_writer = csv.writer(file)
        csv_writer.writerow(['prediction', 'label'])
        for prediction, label in zip(test_predicted_labels, labels):
            csv_writer.writerow([prediction, label])
